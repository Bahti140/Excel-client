...
const CourseContent: FC<Props> = ({
  ...
  const newContentHandler = (item: any) => {
    if (
      ...
      if (courseContentData.length > 0) {
        ...
        // use the last videosectioon if available, else use user input
        if (lastVideoSection) {
  ...
  const handleOptions = () => {
    if (
      ...
      toast.error("section caan't be empty!");
    } else {
      setActive(active + 1);
      handleCourseSubmit();
    }
                          ...
                          <AiOutlineDelete
                            className={`${
                              linkIndex === 0
                                ? "cursor-no-drop"
                                : "cursor-pointer"
                            } text-black dark:text-white text-[20]`}
                            onClick={() =>
                              linkIndex === 0
                                ? null
